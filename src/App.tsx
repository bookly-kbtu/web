...
export default function App() {
  ...
  // Refresh only when opening the profile, avoiding duplicate token rotations in StrictMode.
    ...
    if (session) {
      try {
        ...
        if (payload.exp * 1000 < Date.now() + 30000)
          current = {
            ...session,
            ...(await post<Tokens>("/auth/refresh", {
  ...
  return (
    <>
      <header className="header">
        ...
        <button className="account" onClick={openProfile}>
          <UserRound size={17} />
      ...
      <main className={layout === "map" ? "map-mode" : ""}>
        <section className="intro">
          ...
          <div className="intro-bottom">
            <span>
              {view === "saved" ? "ВАША ЛИЧНАЯ ПОДБОРКА" : "НАЙДИТЕ СВОЁ МЕСТО"}
            </span>
        ...
        <section id="catalog" className="catalog">
          ...
          {layout === "map" ? (
            ...
            <div className="empty">
              <span className="empty-icon">
                {view === "saved" ? <Heart size={28} /> : <Search size={28} />}
              </span>
              <h3>
                {view === "saved"
